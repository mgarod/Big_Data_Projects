...
def create_ProjectGraph():
    ...
    statement1 = """
        LOAD CSV WITH HEADERS FROM 'file://%s/Data//projects.csv' AS line
    ...
    statement2 = """
        LOAD CSV WITH HEADERS FROM 'file://%s/Data//projects.csv' AS line
        MATCH (a:Person { User_id: TOINT(line.User_id) }),
            (b:projects { projectname:line.Project })
...
def create_Relationship():
    ...
    statement1 = """
        ...
        MATCH (a:Person { User_id: TOINT(line.User_id) }),
            (b:Organization { name:line.Organization })
        CREATE (a)-[:Work_at]->(b)
